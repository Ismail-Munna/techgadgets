...
const PREFERRED_CATEGORY_ORDER = ["Audio", "Wearables", "Computers"];
...
export default function ProductsCatalog({
  ...
  const categories = useMemo(() => {
    ...
    const orderedCategories = [
      ...PREFERRED_CATEGORY_ORDER.filter((category) =>
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      const shortDescription = normalizeSearchValue(product.shortDescription);
    ...
    <>
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
          <button
            type="button"
            className="flex items-center px-4 py-2 border border-gray-300 rounded-full text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 whitespace-nowrap"
          >
          ...
          {categories.map((category) => {
            ...
            return (
              <button
                ...
                className={
                  isActive
                    ? "px-4 py-2 bg-indigo-50 text-indigo-700 rounded-full text-sm font-medium whitespace-nowrap"
                    : "px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-full text-sm font-medium whitespace-nowrap"
                }
              >
                {category}
              </button>
